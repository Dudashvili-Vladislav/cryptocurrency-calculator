                                ...
                                >
                                    ...
                                    >
                                        <td class="field__descriptions">
                                            <p v-if="!isEditingMargins">{{ margin.Client_Id }}</p>
                                            <input v-else v-model="margin.Client_Id"/>
                                        </td>

                                        <td class="field__descriptions">
                                            <p v-if="!isEditingMargins">{{ margin.availableFunds }}</p>
                                            <input
                                                v-else
                                        ...
                                        <td class="field__descriptions">
                                            <p v-if="!isEditingMargins">{{ margin.currency }}</p>
                                            <input v-else v-model="margin.currency"/>
                                        </td>

                                        <td class="field__descriptions">
                                            <p v-if="!isEditingMargins">{{ margin.marginRequirements }}</p>
                                            <input
                                                v-else
                                        ...
                                        <td class="field__descriptions">
                                            <p v-if="!isEditingMargins">{{ margin.fundsNotional }}</p>
                                            <input
                                                v-else
    ...
    methods: {
        ...
        async getUserForTable(userId) {
            try {
                ...
                let [
                    ...
                    this.fetchCleintTableInfoByTab({userId, url: "/admin/margins"}),
                    this.fetchCleintTableInfoByTab({userId, url: "/admin/positions"}),
                    this.fetchCleintTableInfoByTab({userId, url: "/admin/deals"}),
                    this.fetchCleintTableInfoByTab({userId, url: "/admin/orders"}),
                ...
                if (
                    ...
                    console.log("Юзер поменялся пока ждал запрос, завершаю");
                    return;
                }
            ...
            setTimeout(() => {
                if (this.currentUser === userId && !this.isEditing) {
                    ...
                    console.log(
                        "Юзер поменялся или включилось редактирование, завершаю рекурсивность"
                    );
                }
            }, 2000);
        ...
        convertFunds(response) {
            try {
                ...
                console.error("PARSE FUNDS ERR", err);
                return [];
            }
        },
        ...
        convertDeals(response) {
            try {
                ...
                console.error("PARSE DEALS ERR", err);
                return [];
            }
        },
        ...
        convertPositions(response) {
            ...
            try {
                ...
                console.error("PARSE POSITIONS ERR", err);
                return [];
            }
        },
        ...
        convertMargins(response) {
            ...
            try {
                ...
                console.error(`PARSE MARGINS ERR`, err);
                return [];
            }
        },
